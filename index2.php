...
if (!isset($_SESSION['conname'])) {
    header("Location: index.html");
}else{
  $filter = $_SESSION['conname'];
...
<head>
  ...
  <title>Wema App - Emergency Contact Homepage</title>
...
$sql = "SELECT `assistance`.`Assistance_ID`, `assistance`.`Emergency_Type`, `assistance`.`Report`, `assistance`.`Safety_Preferences`, `assistance`.`Long`, `assistance`.`Lat`, `assistance`.`Status`, `assistance`.`Created_At`, `users`.`Fullname`, `contacts`.`Contact_ID` FROM `assistance` JOIN `users` ON `users`.`User_ID` = `assistance`.`User_ID` JOIN `contacts` ON `contacts`.`User_ID` = `users`.`User_ID` WHERE `contacts`.`Emergency_Contact_ID` = '$filter'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
